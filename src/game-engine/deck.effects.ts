/**
 * Deck Effects - Passive abilities based on deck mythology composition
 *
 * Effects:
 * - Norse: Random card in hand gains +1 at turn start when behind
 * - Polynesian: Random card in hand gains +1 when terrain is added (once per round)
 * - Japanese: Random card in hand gains +1 when a card is debuffed (once per round)
 */

import { GameState, Player, BoardPosition } from "../types/game.types";
import { InGameCard } from "../types/card.types";
import {
  BaseGameEvent,
  CardPowerChangedEvent,
  EVENT_TYPES,
} from "../types/game-engine.types";
import { addTempBuff, addTempDebuff, updateCurrentPower } from "./ability.utils";
import { randomInt } from "./simulation.rng";

// Sentinel position for cards in hand (not on board)
const HAND_POSITION: BoardPosition = { x: -1, y: -1 };

/**
 * Calculate the current round number from turn number.
 * Each round consists of 2 turns (one per player).
 */
export function getCurrentRound(turnNumber: number): number {
  return Math.ceil(turnNumber / 2);
}

/**
 * Calculate score on the fly by counting board cards owned by each player.
 * This avoids needing to maintain score state during gameplay.
 */
export function calculateScoreOnTheFly(
  gameState: GameState,
  playerId: string
): { playerScore: number; opponentScore: number } {
  let playerScore = 0;
  let opponentScore = 0;

  for (const row of gameState.board) {
    for (const cell of row) {
      if (cell.card) {
        if (cell.card.owner === playerId) {
          playerScore++;
        } else {
          opponentScore++;
        }
      }
    }
  }

  return { playerScore, opponentScore };
}

/**
 * Check if opponent is currently leading (has more cards on board).
 */
export function isOpponentLeading(
  gameState: GameState,
  playerId: string
): boolean {
  const { playerScore, opponentScore } = calculateScoreOnTheFly(
    gameState,
    playerId
  );
  return opponentScore > playerScore;
}

/**
 * Get a random card from a player's hand.
 * Returns null if hand is empty.
 */
export function getRandomHandCard(
  gameState: GameState,
  playerId: string
): InGameCard | null {
  const player =
    gameState.player1.user_id === playerId
      ? gameState.player1
      : gameState.player2;

  if (player.hand.length === 0) {
    return null;
  }

  const randomIndex = randomInt(player.hand.length);
  const cardId = player.hand[randomIndex];

  return gameState.hydrated_card_data_cache?.[cardId] || null;
}

/**
 * Get the player object from the game state.
 */
export function getPlayer(gameState: GameState, playerId: string): Player {
  return gameState.player1.user_id === playerId
    ? gameState.player1
    : gameState.player2;
}

/**
 * Apply the Norse deck effect: +1 to a random card in hand at turn start when behind.
 *
 * @returns Events generated by the effect, or empty array if not triggered
 */
export function applyNorseDeckEffect(
  gameState: GameState,
  playerId: string
): BaseGameEvent[] {
  const player = getPlayer(gameState, playerId);

  // Check if player has Norse deck effect
  if (player.deck_effect !== "norse") {
    return [];
  }

  // Check if opponent is currently leading at turn start
  if (!isOpponentLeading(gameState, playerId)) {
    return [];
  }

  // Get a random card from the player's hand
  const randomCard = getRandomHandCard(gameState, playerId);
  if (!randomCard) {
    return [];
  }

  // Apply +1 buff to the random hand card
  const events: BaseGameEvent[] = [];
  events.push(
    addTempBuff(randomCard, 1000, 1, {
      name: "Fated Resolve",
      animation: "norse-deck-effect",
      position: HAND_POSITION,
    })
  );

  // Update the card's current power
  randomCard.current_power = updateCurrentPower(randomCard);

  // Also update in the cache if present
  if (
    gameState.hydrated_card_data_cache &&
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id]
  ) {
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id] =
      randomCard;
  }

  return events;
}

/**
 * Apply the Polynesian deck effect: +1 to a random card in hand when terrain is added.
 * This is a once-per-round effect.
 *
 * @param actingPlayerId - The player who triggered the terrain effect (either player can trigger it)
 * @returns Events generated by the effect, or empty array if not triggered
 */
export function applyPolynesianDeckEffect(
  gameState: GameState,
  beneficiaryPlayerId: string
): BaseGameEvent[] {
  const player = getPlayer(gameState, beneficiaryPlayerId);

  // Check if player has Polynesian deck effect
  if (player.deck_effect !== "polynesian") {
    return [];
  }

  // Check once-per-round limit
  const currentRound = getCurrentRound(gameState.turn_number);
  if (
    player.deck_effect_state &&
    player.deck_effect_state.last_triggered_round >= currentRound
  ) {
    return [];
  }

  // Get a random card from the player's hand
  const randomCard = getRandomHandCard(gameState, beneficiaryPlayerId);
  if (!randomCard) {
    return [];
  }

  // Update the trigger state
  if (!player.deck_effect_state) {
    player.deck_effect_state = { last_triggered_round: 0 };
  }
  player.deck_effect_state.last_triggered_round = currentRound;

  // Apply +1 buff to the random hand card
  const events: BaseGameEvent[] = [];
  events.push(
    addTempBuff(randomCard, 1000, 1, {
      name: "ʻĀina Bound",
      animation: "polynesian-deck-effect",
      position: HAND_POSITION,
    })
  );

  // Update the card's current power
  randomCard.current_power = updateCurrentPower(randomCard);

  // Also update in the cache if present
  if (
    gameState.hydrated_card_data_cache &&
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id]
  ) {
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id] =
      randomCard;
  }

  return events;
}

/**
 * Apply the Japanese deck effect: +1 to a random card in hand when a card is debuffed.
 * This is a once-per-round effect.
 *
 * @returns Events generated by the effect, or empty array if not triggered
 */
export function applyJapaneseDeckEffect(
  gameState: GameState,
  beneficiaryPlayerId: string
): BaseGameEvent[] {
  const player = getPlayer(gameState, beneficiaryPlayerId);

  // Check if player has Japanese deck effect
  if (player.deck_effect !== "japanese") {
    return [];
  }

  // Check once-per-round limit
  const currentRound = getCurrentRound(gameState.turn_number);
  if (
    player.deck_effect_state &&
    player.deck_effect_state.last_triggered_round >= currentRound
  ) {
    return [];
  }

  // Get a random card from the player's hand
  const randomCard = getRandomHandCard(gameState, beneficiaryPlayerId);
  if (!randomCard) {
    return [];
  }

  // Update the trigger state
  if (!player.deck_effect_state) {
    player.deck_effect_state = { last_triggered_round: 0 };
  }
  player.deck_effect_state.last_triggered_round = currentRound;

  // Apply +1 buff to the random hand card
  const events: BaseGameEvent[] = [];
  events.push(
    addTempBuff(randomCard, 1000, 1, {
      name: "Measured Technique",
      animation: "japanese-deck-effect",
      position: HAND_POSITION,
    })
  );

  // Update the card's current power
  randomCard.current_power = updateCurrentPower(randomCard);

  // Also update in the cache if present
  if (
    gameState.hydrated_card_data_cache &&
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id]
  ) {
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id] =
      randomCard;
  }

  return events;
}

/**
 * Check all players for Polynesian deck effect and apply if terrain was added.
 * Called when a terrain effect is added to a tile.
 */
export function triggerTerrainDeckEffects(
  gameState: GameState
): BaseGameEvent[] {
  const events: BaseGameEvent[] = [];

  // Check both players for Polynesian deck effect
  events.push(...applyPolynesianDeckEffect(gameState, gameState.player1.user_id));
  events.push(...applyPolynesianDeckEffect(gameState, gameState.player2.user_id));

  return events;
}

/**
 * True if the given event batch contains a negative temporary effect actually
 * applied to a card. Immune targets emit a `protected` power change with no
 * `isNegativeEffect` flag and correctly do NOT count as a debuff.
 */
export function batchContainsDebuff(events: BaseGameEvent[]): boolean {
  return events.some(
    (event) =>
      event.type === EVENT_TYPES.CARD_POWER_CHANGED &&
      (event as CardPowerChangedEvent).isNegativeEffect === true
  );
}

/**
 * Check all players for the Japanese deck effect and apply if a card was
 * debuffed. Called with the events produced by the stage that just ran; the
 * passive fires only when that stage actually debuffed a card.
 */
export function triggerDebuffDeckEffects(
  gameState: GameState,
  producedEvents: BaseGameEvent[]
): BaseGameEvent[] {
  if (!batchContainsDebuff(producedEvents)) {
    return [];
  }

  return triggerJapaneseDeckEffects(gameState);
}

/**
 * Check all players for the Japanese deck effect and apply unconditionally.
 * Used by triggerDebuffDeckEffects once it has confirmed a debuff landed, and
 * directly by the tile-curse site, which knows a curse was transferred but
 * emits its own power-change event rather than going through addTempDebuff.
 */
export function triggerJapaneseDeckEffects(
  gameState: GameState
): BaseGameEvent[] {
  const events: BaseGameEvent[] = [];

  // Check both players for Japanese deck effect
  events.push(...applyJapaneseDeckEffect(gameState, gameState.player1.user_id));
  events.push(...applyJapaneseDeckEffect(gameState, gameState.player2.user_id));

  return events;
}

/**
 * Tower Encounter Modifier: Poison.
 *
 * At the start of the afflicted player's turn, a random card in their hand loses
 * power. Deliberately shaped like applyNorseDeckEffect (its mirror image): pick a
 * random hand card, apply a hand-scoped temporary effect, resync the hydration
 * cache so the client's next push shows the new power.
 *
 * Only the human player is poisoned. The modifier exists to pressure the player's
 * hand; applying it to the AI as well would cancel out and make the floor read as
 * unchanged.
 *
 * Two traps this deliberately avoids:
 *  - the magnitude is passed NEGATIVE, matching every other addTempDebuff caller
 *    (it normalizes to always-negative per side; a positive here would still work
 *    but would break the convention the sign-normalization comment relies on);
 *  - the position is HAND_POSITION, so the floating text renders on the hand card
 *    and never leaks onto a board tile.
 *
 * @returns Events generated by the effect, or empty array if not applicable
 */
export function applyTowerPoisonEffect(
  gameState: GameState,
  playerId: string
): BaseGameEvent[] {
  const poison = gameState.tower_context?.modifiers?.find(
    (modifier) => modifier.type === "poison"
  );
  if (!poison) {
    return [];
  }

  // player1 is always the human in tower games; the AI is not poisoned.
  if (gameState.player1.user_id !== playerId) {
    return [];
  }

  const randomCard = getRandomHandCard(gameState, playerId);
  if (!randomCard) {
    return [];
  }

  const amount = Number(poison.value);
  const magnitude = Number.isFinite(amount) && amount > 0 ? amount : 1;

  const events: BaseGameEvent[] = [];
  events.push(
    addTempDebuff(randomCard, 1000, -magnitude, {
      name: poison.label || "Poison",
      animation: "debuff",
      position: HAND_POSITION,
    })
  );

  randomCard.current_power = updateCurrentPower(randomCard);

  if (
    gameState.hydrated_card_data_cache &&
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id]
  ) {
    gameState.hydrated_card_data_cache[randomCard.user_card_instance_id] =
      randomCard;
  }

  return events;
}
