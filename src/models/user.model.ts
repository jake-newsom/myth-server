...
const UserModel = {
  async create({ username, email, password }: UserCreateInput): Promise<User> {
    ...
    const query = `
      INSERT INTO "users" (username, email, password_hash, in_game_currency, created_at, last_login)
      VALUES ($1, $2, $3, $4, NOW(), NOW())
      RETURNING user_id, username, email, in_game_currency, created_at, last_login as last_login_at;
    `;
  ...
  async findById(userId: string): Promise<User | null> {
    const query = `SELECT user_id, username, email, in_game_currency, created_at, last_login as last_login_at FROM "users" WHERE user_id = $1;`;
