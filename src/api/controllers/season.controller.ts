...
export const getPreviousStandings = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    const data = await SeasonSoulsService.getPreviousStandings();
    return res.status(200).json(data);
  } catch (error) {
    console.error("Error getting previous season standings:", error);
    return res.status(500).json({ status: "error", message: "Internal server error" });
  }
};

export const getPreviousLeaderboard = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    const data = await SeasonSoulsService.getPreviousLeaderboard(page, limit);
    return res.status(200).json(data);
  } catch (error) {
    console.error("Error getting previous season leaderboard:", error);
    return res.status(500).json({ status: "error", message: "Internal server error" });
  }
};
